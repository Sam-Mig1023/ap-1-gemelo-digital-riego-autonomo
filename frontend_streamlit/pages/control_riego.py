...
def render():
    ...
    if status:
        ...
        st.warning("⚠️ API offline — mostrando estado de zonas desde SQLite")
    ...
    with col_stop:
        ...
        if st.button("🛑 DETENER TODO EL RIEGO", type="primary",
            ...
            st.error(f"🛑 {msg}")
        if not puede_parar:
            st.caption(f"⚠️ El rol **{rol}** no puede activar la parada de emergencia.")
    ...
    if dec:
        ...
        if st.button("▶️ Ejecutar Decisión RL", disabled=not puede_exec,
        ...
        if not puede_exec:
            st.caption(f"⚠️ El rol **{rol}** no puede ejecutar riegos.")
    else:
        ...
        if not pendientes.empty and "pendiente" in set(pendientes["estado"].tolist()):
            ...
            st.info("ℹ️ Ve al módulo **Agente RL**, ejecuta una inferencia y aprueba la decisión primero.")

    st.divider()

    # ── Disparo manual ────────────────────────────────────────────────────────
    st.subheader("🖐️ Disparo Manual de Riego")
    ...
    if not puede_manual:
        st.caption(f"⚠️ El rol **{rol}** no tiene permiso para riego manual.")
