...
export const FieldGISMap: React.FC<FieldGISMapProps> = ({
  ...
  const getZoneFillColor = (zone: ManagementZone) => {
    switch (activeLayer) {
      case 'cwsi':
        if (zone.cwsi > 0.65) return 'rgba(239, 68, 68, 0.75)';
        if (zone.cwsi > 0.40) return 'rgba(245, 158, 11, 0.70)';
        if (zone.cwsi > 0.20) return 'rgba(16, 185, 129, 0.65)';
      ...
      case 'soil_texture':
        if (zone.soilTexture === 'sandy_loam') return 'rgba(217, 119, 6, 0.70)';
        if (zone.soilTexture === 'silt_loam') return 'rgba(13, 148, 136, 0.70)';
        return 'rgba(180, 83, 9, 0.70)';
      case 'vri_rate':
  ...
  return (
    <div className="space-y-4">
      ...
      <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-5 gap-3">
        ...
        <div className="bg-white dark:bg-slate-900/90 border border-slate-200 dark:border-slate-800 p-3.5 rounded-xl shadow-sm">
          ...
          <div className="mt-1 flex items-baseline gap-2">
            <span className="text-xl sm:text-2xl font-bold text-slate-900 dark:text-white">{avgCwsi}</span>
      ...
      <div className="bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 rounded-2xl overflow-hidden shadow-lg dark:shadow-2xl">
        
        {/* Map Control Bar */}
        <div className="bg-slate-50 dark:bg-slate-950/80 border-b border-slate-200 dark:border-slate-800 p-3 sm:p-4 flex flex-wrap items-center justify-between gap-3">
              ...
              <button
                ...
                className={`px-2.5 py-1 text-xs font-medium rounded-lg border transition-all ${
                  activeLayer === layer.id
                    ? 'bg-slate-200 dark:bg-slate-800 text-slate-900 dark:text-white border-emerald-500/50 shadow-sm font-semibold'
                    : 'bg-slate-100 dark:bg-slate-900/60 text-slate-600 dark:text-slate-400 border-slate-200 dark:border-slate-800 hover:border-slate-300 dark:hover:border-slate-700 hover:text-slate-900 dark:hover:text-slate-200'
                }`}
              >
                {layer.label}
              </button>
        ...
        <div className="relative p-4 sm:p-6 bg-slate-50 dark:bg-radial dark:from-slate-900 dark:via-slate-950 dark:to-slate-950 flex flex-col lg:flex-row gap-6 items-center justify-between">
          
          {/* Geospatial Canvas SVG */}
          <div className="relative w-full max-w-xl aspect-square bg-white dark:bg-slate-950/90 rounded-2xl border border-slate-200 dark:border-slate-800 p-2 shadow-inner flex items-center justify-center overflow-hidden">
            
            {/* Background Map Grid Pattern */}
            <div className="absolute inset-0 opacity-10 dark:opacity-10 bg-[linear-gradient(to_right,#10b981_1px,transparent_1px),linear-gradient(to_bottom,#10b981_1px,transparent_1px)] bg-[size:24px_24px]" />
            ...
            <svg viewBox="0 0 500 500" className="w-full h-full">
              ...
              <circle cx="250" cy="250" r="220" fill="none" stroke="#94a3b8" strokeWidth="2" strokeDasharray="4 4" className="dark:stroke-slate-600" />
              <circle cx="250" cy="250" r="160" fill="none" stroke="#cbd5e1" strokeWidth="1.5" className="dark:stroke-slate-700" />
              <circle cx="250" cy="250" r="100" fill="none" stroke="#cbd5e1" strokeWidth="1.5" className="dark:stroke-slate-700" />

              {/* 4 Quadrants Management Zones */}
              <path
                d="M 250 250 L 50 250 A 200 200 0 0 1 250 50 Z"
                fill={getZoneFillColor(zones[0])}
                stroke={selectedZone?.id === zones[0].id ? '#10b981' : '#64748b'}
              ...
              <path
                ...
                stroke={selectedZone?.id === zones[1].id ? '#10b981' : '#64748b'}
              ...
              <path
                ...
                stroke={selectedZone?.id === zones[2].id ? '#10b981' : '#64748b'}
              ...
              <path
                ...
                stroke={selectedZone?.id === zones[3].id ? '#10b981' : '#64748b'}
          ...
          <div className="w-full lg:w-96 space-y-3">
            {selectedZone ? (
              <div className="bg-slate-50 dark:bg-slate-950 p-4 rounded-xl border border-slate-200 dark:border-slate-800 space-y-4 shadow-sm">
                ...
                <div className="bg-gradient-to-br from-emerald-50 dark:from-emerald-950/40 to-white dark:to-slate-900 p-3.5 rounded-xl border border-emerald-500/30 space-y-2.5">
                  <div className="flex items-center justify-between">
