...
export const Header: React.FC<HeaderProps> = ({
  ...
  const getRoleBadge = (role: UserRole) => {
    switch (role) {
      case 'superadmin':
        return { label: t('header.superadmin'), color: 'bg-purple-500/20 text-purple-300 dark:text-purple-300 border-purple-500/40' };
      case 'agronomist':
        return { label: t('header.agronomist'), color: 'bg-emerald-500/20 text-emerald-300 dark:text-emerald-300 border-emerald-500/40' };
      case 'farmer':
        return { label: t('header.farmer'), color: 'bg-amber-500/20 text-amber-300 dark:text-amber-300 border-amber-500/40' };
      case 'field_technician':
        return { label: t('header.fieldTechnician'), color: 'bg-blue-500/20 text-blue-300 dark:text-blue-300 border-blue-500/40' };
      case 'rl_agent_system':
        return { label: t('header.rlAgent'), color: 'bg-rose-500/20 text-rose-300 dark:text-rose-300 border-rose-500/40' };
    }
  };

  const getCropStageLabel = (stage: string) => {
    return t(`header.cropStages.${stage}`) || stage;
  };
      ...
      <div className="max-w-full px-4 sm:px-6 xl:px-8 py-3 flex flex-wrap lg:flex-nowrap items-center justify-between gap-3 lg:gap-4">
          ...
          <div className="min-w-0">
            <div className="flex items-center gap-2 min-w-0">
              <h1 className="text-base sm:text-lg font-bold text-slate-900 dark:text-white tracking-tight flex items-center gap-2 min-w-0 whitespace-nowrap overflow-hidden">
                {t('header.title')} <span className="text-emerald-400 shrink-0">{t('header.titleSuffix')}</span>
              </h1>
              <span className="hidden xl:inline-flex shrink-0 px-2 py-0.5 text-xs font-semibold rounded bg-emerald-500/10 text-emerald-400 border border-emerald-500/30">
                {t('header.rlCoreBadge')}
              </span>
            </div>
            <p className="text-xs text-slate-600 dark:text-slate-400 font-medium truncate">
              {field.name} • {field.cropName} ({getCropStageLabel(field.cropStage)})
            </p>
          </div>
        </div>
        ...
        <div className="flex flex-wrap lg:flex-nowrap w-full lg:w-auto items-center justify-end gap-1.5 sm:gap-2 xl:gap-2.5 shrink-0">
          ...
          >
            ...
            <span className="font-bold">{language === 'es' ? t('header.spanish') : t('header.english')}</span>
          </button>

          <button
            onClick={toggleTheme}
            className="flex shrink-0 items-center gap-1.5 px-2.5 py-1.5 rounded-lg border text-xs font-medium transition-all bg-slate-100 dark:bg-slate-950 text-slate-700 dark:text-slate-300 border-slate-300 dark:border-slate-700 hover:border-slate-400 dark:hover:border-slate-600 hover:bg-slate-200 dark:hover:text-slate-100"
            title={t('header.switchTheme', { theme: theme === 'dark' ? t('header.themeLight').toLowerCase() : t('header.themeDark').toLowerCase() })}
          >
            {theme === 'dark' ? (
              <>
          ...
          <div className="flex shrink-0 items-center bg-slate-100 dark:bg-slate-950/80 border-slate-300 dark:border-slate-800 p-1 rounded-lg border">
            <span className="text-xs font-medium text-slate-600 dark:text-slate-400 px-1.5 xl:px-2 flex items-center gap-1">
              ...
              <span className="hidden xl:inline">{t('header.modeLabel')}</span>
            </span>
            <button
              id="mode-manual-btn"
            ...
            >
              ...
              <span className="hidden sm:inline">{t('header.modeAutonomous')}</span>
              <span className="sm:hidden">{t('header.modeAutonomousShort')}</span>
            </button>
          </div>
          ...
          >
            {isOnline ? (
              ...
              <>
                ...
                <span className="sm:hidden">{t('header.offlineWithCount', { count: offlineQueueCount })}</span>
                <span className="hidden sm:inline">{t('header.ruralOffline', { count: offlineQueueCount })}</span>
              </>
            )}
          </button>
          ...
          <div className="flex shrink-0 items-center gap-1.5 bg-slate-100 dark:bg-slate-950/80 border-slate-300 dark:border-slate-800 px-2 xl:px-2.5 py-1 rounded-lg border max-w-[320px] xl:max-w-none">
            ...
            >
              <option value="superadmin" className="bg-slate-100 dark:bg-slate-900 text-slate-900 dark:text-white">{t('header.superadmin')}</option>
              <option value="agronomist" className="bg-slate-100 dark:bg-slate-900 text-slate-900 dark:text-white">{t('header.agronomist')}</option>
              <option value="farmer" className="bg-slate-100 dark:bg-slate-900 text-slate-900 dark:text-white">{t('header.farmer')}</option>
              <option value="field_technician" className="bg-slate-100 dark:bg-slate-900 text-slate-900 dark:text-white">{t('header.fieldTechnician')}</option>
              <option value="rl_agent_system" className="bg-slate-100 dark:bg-slate-900 text-slate-900 dark:text-white">{t('header.rlAgentSystem')}</option>
            </select>
