      ...
      <div className="bg-slate-900 border border-slate-800 p-4 sm:p-5 rounded-2xl space-y-4 shadow-xl">
        <div className="flex items-center justify-between border-b border-slate-800 pb-3">
          <div className="flex items-center gap-2">
            ...
            <h3 className="text-sm font-bold text-white">Matriz de Permisos Granulares por Rol de Usuario</h3>
          </div>
            ...
            <tbody className="divide-y divide-slate-800 text-slate-300">
              {permissionsMatrix.map((item, i) => (
                <tr key={i} className="hover:bg-slate-800/50 transition-colors">
                  <td className="p-3 font-medium text-slate-200">{item.permission}</td>
      ...
      <div className="bg-slate-900 border border-slate-800 p-4 sm:p-5 rounded-2xl space-y-4 shadow-xl">
        <div className="flex flex-wrap items-center justify-between gap-3 border-b border-slate-800 pb-3">
          <div className="flex items-center gap-2">
            ...
            <h3 className="text-sm font-bold text-white">Log de Auditoría Inmutable (Trazabilidad SHA-256)</h3>
          </div>
          ...
          <div className="relative">
            ...
            <input
              ...
              className="bg-slate-950 border border-slate-700 rounded-lg pl-8 pr-3 py-1.5 text-xs text-white placeholder-slate-500 focus:outline-none focus:border-indigo-500 w-48 sm:w-64"
            />
          </div>
        </div>
            ...
            <tbody className="divide-y divide-slate-800 text-slate-300">
              {filteredLogs.map((log) => (
                <tr key={log.id} className="hover:bg-slate-800/50 transition-colors">
                  ...
                  <td className="p-3">
                    <span className="font-semibold text-white block">{log.userEmail}</span>
                  ...
                  <td className="p-3 text-slate-300 font-mono text-[11px]">{log.resource}</td>
                  <td className="p-3 text-slate-300 max-w-xs">{log.details}</td>
